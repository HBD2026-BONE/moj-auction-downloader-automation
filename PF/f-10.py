...
def process_and_download_data(driver):
    ...
    wait = WebDriverWait(driver, 5) # 稍微拉長一點點，避免 RWD 渲染太慢
    ...
    for index, row in enumerate(rows):
        try:
            ...
            # 🔥 關鍵修正：多重防禦機制抓取案號，避免 RWD 斷層
            try:
                # 方案 A: 用 data-label 包含 '案號' 定位
                case_td = row.find_element(By.XPATH, ".//td[contains(@data-label, '案號')]")
                full_td_text = case_td.text.strip()
            except Exception:
                try:
                    # 方案 B: 直接尋找文字內含有特定格式（例如 3碼+分署代碼）的 td
                    # 如果網站是法拍/拍賣系統，常有固定格式。這裡先擴大範圍找尋任何有文字的第 2 或第 3 個 td
                    all_tds = row.find_elements(By.XPATH, "./td")
                    if len(all_tds) >= 2:
                        # 通常第一欄是序號或勾選方塊，我們取後面的欄位來試
            ...
            # 安全防錯：如果 RWD 真的把 td 藏起來了，我們用序號當備用名稱，絕對不讓程式崩潰
            if not case_no:
            ...
            # 2. 點擊進入新分頁
            try:
                # 🔥 修正：RWD 模式下原本的 td[contains(@data-th, '照片')] 可能失效，改成直接找行內包含 Detail 的超連結
    ...
    if total_tasks > 0:
        ...
        print(f"🎉【全部下載完成】成功儲存 {success_count} / {total_tasks} 個檔案！")
    else:
        print("\n[!] 未發現任何可下載的檔案網址。")
