...
def run_task():
    ...
    for attempt in range(max_retries):
        try:
            ...
            # 填寫表單
            kind_el = wait.until(EC.element_to_be_clickable((By.ID, "THE_USE")))    # 種類
            Select(kind_el).select_by_visible_text(THE_USE_TEXT)
            dept_el = wait.until(EC.element_to_be_clickable((By.ID, "EXEC_DEPT_ID"))) # 分署
            Select(dept_el).select_by_index(DEPT_INDEX)
            driver.find_element(By.ID, "OPEN_BID_TIME_S").send_keys(DATE_START) # 起始日
            driver.find_element(By.ID, "OPEN_BID_TIME_E").send_keys(DATE_END)   # 結束日
            ...
            if not rows:
                ...
                for index, row in enumerate(rows):
                    try:
                        ...
                        # a. 廠牌抓取：增加多種可能的關鍵字 (廠牌、廠牌型式、TOYOTA牌...)
